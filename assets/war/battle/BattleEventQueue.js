(function(w){
  ...
  class BattleEventQueue{
    constructor(runner,onIdle){this.runner=runner||null;this.onIdle=onIdle||null;this.items=[];this.running=false;this.epoch=0}
    ...
    async drain(){
      ...
      finally{this.running=false;if(this.items.length)this.drain();else if(epoch===this.epoch)this.onIdle?.()}
    }
  }
