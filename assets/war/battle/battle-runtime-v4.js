(function(w){
  ...
  const C=w.TopKingBattleCore||{},state={scene:null,adapter:null,queue:null,ready:false,initializing:null,pending:null,hpCallback:null,generation:0};
  ...
  async function init(){
    ...
    const promise=(async()=>{
      ...
      try{
        await scene.init(event=>{if(generation!==state.generation)return;const shown=activeHp(event.snapshot||{});if(shown.side===event.defender)emitHp(event.remainingHp,event.maxHp,{animated:true,event})});
  ...
  function destroy(){state.generation++;state.queue?.clear();state.scene?.destroy();state.adapter?.reset();Object.assign(state,{scene:null,adapter:null,queue:null,pending:null,hpCallback:null,ready:false,initializing:null})}
