(function(w){
  ...
  async function init(){
    ...
    state.adapter=new C.WarEventAdapter();state.scene=new C.BattleScene({PIXI:w.PIXI,gsap:w.gsap,canvas,host});try{await state.scene.init(event=>{const shown=activeHp(event.snapshot||{});if(shown.side===event.defender)emitHp(event.remainingHp,event.maxHp,{animated:true,event})})}catch(err){host.classList.add('battle-v2-failed');throw err}state.queue=new C.BattleEventQueue(event=>state.scene.play(event));state.ready=true;
  ...
  w.TopKingBattleV4={version:'4.0.3-texture-loader',init,update,destroy};
