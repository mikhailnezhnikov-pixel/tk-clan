(function(w){
  ...
  async function init(){
    ...
    state.adapter=new C.WarEventAdapter();state.scene=new C.BattleScene({PIXI:w.PIXI,gsap:w.gsap,canvas,host});await state.scene.init(event=>{const shown=activeHp(event.snapshot||{});if(shown.side===event.defender)emitHp(event.remainingHp,event.maxHp,{animated:true,event})});state.queue=new C.BattleEventQueue(event=>state.scene.play(event));state.ready=true;
  ...
  w.TopKingBattleV4={version:'4.0.1-milestone-1',init,update,destroy};
