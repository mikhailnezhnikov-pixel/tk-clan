(function(w){
  ...
  class WarEventAdapter{
    ...
    identity(war){return [war?.opponent||'',war?.started_at||war?.start_at||'',war?.war_id||war?.id||''].join('|')}
    ...
    ingest(war){
      ...
      const events=[];
      if(state.enemy!=null&&this.prev.enemy!=null&&state.enemy<this.prev.enemy)events.push({type:'attack',attacker:'ours',defender:'enemy',damage:Math.max(0,this.prev.enemy-state.enemy),remainingHp:state.enemy,maxHp:state.enemyMax,snapshot:war});
      if(state.ours!=null&&this.prev.ours!=null&&state.ours<this.prev.ours)events.push({type:'attack',attacker:'enemy',defender:'ours',damage:Math.max(0,this.prev.ours-state.ours),remainingHp:state.ours,maxHp:state.oursMax,snapshot:war});
