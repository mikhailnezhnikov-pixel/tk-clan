(function(w){
  ...
  class Fighter{
    constructor({PIXI,textures,side='left',height=310,name=''}){
      ...
      this.sprite=new PIXI.Sprite(this.textures.idle||PIXI.Texture.EMPTY);
      this.sprite.anchor.set(.5,1);
      this.pose.addChild(this.sprite);
    ...
    applyTexture(state){
      const tex=this.textures[state]||this.textures.idle||null;
      this.state=state;
      if(!tex){this.sprite.visible=false;return}
      this.sprite.visible=true;
      this.sprite.texture=tex;
      const h=Math.max(1,tex.height||1),s=this.height/h;
      this.sprite.scale.set(s*this.face,s);
    }
