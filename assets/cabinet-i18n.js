(()=>{
  ...
  const EN={
    ...
    'Список формируется до воскресенья 21:00 GMT+9. Значение 0 в публикацию не попадает; заполнять весь лимит не обязательно.':'The list is prepared until Sunday 21:00 GMT+9. Zero values are not published; you do not need to fill the full limit.',
  ...
  const FA={
    ...
    'Список формируется до воскресенья 21:00 GMT+9. Значение 0 в публикацию не попадает; заполнять весь лимит не обязательно.':'فهرست تا یکشنبه ساعت ۲۱:۰۰ GMT+9 آماده می‌شود. مقادیر صفر منتشر نمی‌شوند و پر کردن کل سقف الزامی نیست.',
