...
MARK="const HK_PITS_PREVIEW_REV = 'pits-runtime-preview-20260920-r15';"
...
if needle in s:
    ...
    needle2="""        const battleResponse=await apiJson(api.battle,'POST');
        {
"""
    replace2="""        const battleResponse=await apiJson(api.battle,'POST');
        pitCanonCaptureRuntimePreview(def,battleResponse);
        {
"""
    if needle2 not in s: raise SystemExit('battle capture anchor missing')
...
for needle in [
    ...
    if needle not in s: raise SystemExit('missing r15 invariant: '+needle)

p.write_text(s,encoding='utf-8')
print('PITS_RUNTIME_PREVIEW_R15_PATCH_OK')
