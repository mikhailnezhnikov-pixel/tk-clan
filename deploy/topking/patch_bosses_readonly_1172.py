...
# deploy-retry: round-trip server verification

def apply_hotfix(text):
    s=text
    REV='bosses-readonly-20260920-r1'
    def req(x,m):
    ...
    for x in ['// @version      1.17.2','HK_NATIVE_LOGIN_GATE_V1 login-gate-20260920-r1','HK_BUREAU_RESOURCES_LIVE_V1 bureau-resources-live-20260920-r1',f"HK_STAGE2J_BOSSES_REV = '{REV}'","{page:'bosses',ru:'Боссы',en:'Bosses'}",'data-content="bosses"','function bossStateRows(','async function refreshBosses(',"documentValue?.player_bosses","documentValue?.player_regional_bosses","documentValue?.boss_battle","GROWTH_HAMSTER_BUDGET_ID = 'cur_cap'","GROWTH_GENERAL_BUDGET_ID = 'item_pit_token'"]:req(x,'Stage 2J missing '+x)
    return s
...
if __name__ == "__main__":
    ...
    print('HK_BOSSES_READONLY_1172_OK')
