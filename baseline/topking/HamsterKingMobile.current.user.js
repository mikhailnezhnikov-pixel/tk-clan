...
(() => {
  ...
  const TEXT = {
    ru: {
      ...
      businessRegular:'Обычная перестановка', businessOptimizer:'Оптимизатор', businessCatalog:'Каталог', optimizer:'Умный оптимизатор', optimizerGoal:'Цель', optimizerPit:'Сила в Яме', optimizerIncome:'Пассивный доход', optimizerEnergy:'Энергия', optimizerBuildings:'Лимит зданий', optimizerRemoveTiers:'Какие тиры можно достать', optimizerInsertTiers:'Какие тиры можно вставить', optimizerReplaceable:'Какие установленные бизнесы можно заменять', optimizerReplaceHint:'Снятая галочка защищает все экземпляры этого бизнеса', optimizerStockAllowed:'Какие складские бизнесы можно вставить', optimizerStockHint:'Выбор доступен вручную для всех тиров', optimizerCalculate:'Рассчитать лучший состав', optimizerApply:'Перенести в план', optimizerRestore:'Вернуть сохранённую схему', optimizerNoPlan:'Сначала считайте бизнесы и рассчитайте состав.', optimizerChanges:'Замен: {n}', optimizerManagers:'Менеджеры: {busy}/{max}', optimizerSafe:'Проверка пройдена', optimizerBlocked:'Запуск заблокирован', optimizerBefore:'Было', optimizerAfter:'Станет', optimizerDifference:'Изменение бонусов',
  ...
  let businessCatalogRoutesLoaded = false;
  ...
  let mapOwnedBuildingIndexPromise = null;
  const mapBuildingStudySubmitDedupe = new Map();
  ...
  const hkRunner = (() => {
    ...
    let pauseResolvers = [];
    const emit = () => { renderRunnerState(); notePublicCollectorRunnerActivity(state); };
  ...
  let publicCollectorActivityPromise=null;
  let publicCollectorActivityLastActive=null;
  let publicCollectorActivityLastSentAt=0;

  async function syncPublicCollectorActivity(active,reason='',force=false) {
    active=!!active;
    if(!licenseState.allowed||!licenseState.token||!licenseState.publicCollectorAuthSync)return false;
    const now=Date.now();
    if(!force&&publicCollectorActivityLastActive===active&&now-publicCollectorActivityLastSentAt<15000)return true;
    if(publicCollectorActivityPromise&&!force)return publicCollectorActivityPromise;
    publicCollectorActivityPromise=(async()=>{
      try{
        const result=await publicCollectorServerPost(
          PUBLIC_COLLECTOR_ACTIVITY_URL,
          {active,ttl_seconds:PUBLIC_COLLECTOR_ACTIVITY_TTL_SECONDS,reason:String(reason||'').slice(0,120)},
          8000
        );
  ...
  async function checkLicense(player, force = false) {
    ...
    licenseCheckPromise = (async () => {
      ...
      if (licenseState.allowed) setTimeout(() => {
        if(!hkRunner.running)void syncPublicCollectorActivity(false,'startup-idle',true);
        scheduleClanShopActualFactsSync();
        const startupTasks = [
  ...
  const HK_BUSINESSES_VERTICAL_RUNNER_REV='businesses-runner-vertical-20260922-r1';
  ...
  const HK_BUSINESSES_KOKKARAS_CANON_REV='businesses-kokkaras-donor-20260922-r1';
  ...
  let explorePlan=null,exploreBusy=false,exploreMeta=null,explorePlayerState=null;
  ...
  function exploreReadSettings(){
    ...
    return exploreSaveSettings({
      ...
      buyMissingMaterials:!!b.querySelector('#hk-ex-buy')?.checked,
  ...
  function exploreBuildingType(...src){
    for(const x of src){if(!x||typeof x!=='object')continue;
  ...
  function exploreBasic(settings,st,meta){
    const allowed=new Set(settings.startTiers.map(Number));let rows=exploreFiltered(settings,st,meta).filter(r=>{const t=Number(r.tier),b=Number(r.battle_level||0),mb=Number(r.max_battle_level||0),l=Number(r.level||0),n=Number(r.next_tier_level||0);
      ...
      .map(r=>{const m=meta?.buildingMeta?.[r.id]||{};return {...r,areaId:String(m.areaId||''),buildingType:String(m.buildingType||''),isInvest:m.isInvest,totalEvents:m.totalEvents};});
  ...
  function exploreE3InitStages(candidate,total,index=1){
  ...
  async function exploreE4BetweenDelay(settings){
  ...
  async function exploreE3ManualBattles(building,settings){
    ...
    for(let round=1;Number(current?.battle_level||0)<Number(current?.max_battle_level||0)&&round<=100;round++){
      await hkRunner.waitIfPaused();const before=Number(current?.battle_level||0);exploreE3Stage('battles',either('Бои','Battles'),'running',before+'/'+Number(current?.max_battle_level||0));hkRunner.setStep(either('Ручной бой','Manual battle')+' · '+before+'/'+Number(current?.max_battle_level||0),0,1);
    ...
    const ok=Number(current?.battle_level||0)>=Number(current?.max_battle_level||0);exploreE3Stage('battles',either('Бои','Battles'),ok?'done':'error',Number(current?.battle_level||0)+'/'+Number(current?.max_battle_level||0));return {ok,building:current,reason:either('Бои не завершены','Battles not completed')};
  }
  async function exploreE3ProcessOne(initial,settings){
    ...
    if(settings.targetTier===8){
      ...
      if(Number(current?.tier||0)<7)return {status:'failed',reason:(res.error?.message||either('MAX не достигнут','MAX was not reached')),building:current};exploreE3Stage('tier',either('Переход по тирам','Tier progression'),'done','MAX');return {status:'completed',building:current};
    }
    for(let guard=0;guard<100;guard++){
      ...
      if(!exploreE3EventsFinished(Array.isArray(current?.events)?current.events:[])){
        ...
        if(res.error&&!exploreE3EventsFinished(current?.events||[]))return {status:'failed',reason:res.error?.message||String(res.error),building:current};if(exploreE3EventsFinished(current?.events||[]))exploreE3Stage('events',either('Исследование событий','Explore events'),'done',exploreTierLabel(tier));await exploreE3Delay(settings,'action');continue;
      }
      ...
      if(Number(current?.tier||0)<=beforeTier)return {status:'failed',reason:res.error?.message||either('Тир не повысился','Tier did not increase'),building:current};exploreE3Stage('tier',either('Переход по тирам','Tier progression'),'done',exploreTierLabel(Number(current?.tier||0)));await exploreE3Delay(settings,'action');
    }
    ...
    try{
      ...
      for(let i=0;i<queue.length;i++){
        ...
        if(result.status==='completed'){
          ...
          log(either('Здание пропущено: ','Building skipped: ')+msg,'warn');
        }else{
      ...
      if(failed){
        ...
        log(either('Исследование остановлено: ','Explore stopped: ')+failed,'warn');
      }else{
        hkRunner.finish(either('Исследование завершено','Explore completed')+' · '+completed+' '+either('готово','done')+(skipped?' · '+skipped+' '+either('пропущено','skipped'):''));
        log(either('Очередь исследования завершена','Explore queue completed')+' · '+completed+'/'+queue.length,'ok');
      }
    }catch(e){
      if(e?.name==='AbortError'){
        ...
        log(either('Исследование остановлено пользователем','Explore stopped by user'),'warn');
      }else{
        ...
        hkRunner.fail(e);
        log(either('Ошибка исследования: ','Explore error: ')+(e?.message||e),'bad');
      }
    }finally{
      exploreBusy=false;
      renderExplore();
  ...
  function renderExplore(){
    ...
    const queueCount=p?Number(p.selected.length||0):0;
    ...
    const planHtml=p?'<div class="hk-cardbox"><b>'+either('План','Plan')+'</b><p><strong>'+either('Будет обработано зданий: ','Buildings to process: ')+queueCount+'</strong></p><p class="hk-muted">'+either('После фильтров','After filters')+': '+p.filteredCount+' · '+either('кандидатов','candidates')+': '+p.candidates.length+'</p>'+
      ...
      (p.meta?.unmapped?'<p class="hk-muted">⚠ '+either('Без района','Without district')+': '+p.meta.unmapped+'</p>':'')+(p.meta?.unknownType?'<p class="hk-muted">⚠ '+either('Тип неизвестен','Unknown type')+': '+p.meta.unknownType+'</p>':'')+'</div>':
    ...
    box.innerHTML='<style>'+
      ...
      '<div class="hk-ex-main">'+
        '<div class="hk-cardbox"><b>'+either('Возможности аккаунта','Account capabilities')+'</b><p class="hk-muted" style="margin:6px 0 0">'+either('Уровень','Level')+': '+(ready?Number(level).toLocaleString(locale()):'—')+' · Remort Consigliere: '+(!ready?'—':c?(Number(c.level||0)+' · '+(c.status==='ACTIVE'?either('активен','active'):either('заблокирован','locked'))):either('не найден','not found'))+' · '+either('Автобои','Auto battles')+': '+(!ready?'—':auto>=0?either('до ','up to ')+exploreTierLabel(auto):either('нет','none'))+'</p><div class="hk-ex-modes">'+modeLine+'</div></div>'+
  ...
  async function acceptBuildingStudy(url, headers, documentValue) {
    ...
    if (!buildingId || !licenseState.allowed) return;
    let areaId=String(mapBuildingAreas.get(buildingId)||'');
    if(!areaId)areaId=await mapResolveBuildingArea(buildingId);
    if (!areaId) return;
    mapRememberBuildingArea(buildingId,areaId);
    ...
    await mapSubmitBuildingStudy(buildingId,areaId,roomCount,hasEvents);
  }
  ...
  const HK_CLAN_SHOP_PURCHASE_HISTORY_CAPTURE_REV='clan-shop-purchase-history-capture-20260922-r1';
  ...
  const PIT_DAILY_API = {
    normal:{stateKeys:['pit'], start:'/player/pit/start', pass:'/player/pit/pass', battle:'/player/pit/battle', respawn:'/player/pit/respawn', finish:'/player/pit/finish'},
    boss:{stateKeys:['pit2','boss_pit'], start:'/player/boss_pit/start', pass:'/player/boss_pit/pass', battle:'/player/boss_pit/battle', respawn:'/player/boss_pit/respawn', finish:'/player/boss_pit/finish'},
    gang:{stateKeys:['pit_pve'], start:'/player/pit_pve/start', pass:'/player/pit_pve/pass', battle:'/player/pit_pve/battle', respawn:'/player/pit_pve/respawn', finish:'/player/pit_pve/finish'}
  };
  ...
  function pitCanonCostQuantity(cost,id,kind='item') {
    const rows=kind==='currency' ? (cost?.currencies||[]) : (cost?.items||[]);
    const row=rows.find(x=>String(x?.id||x?.currency_id||x?.item_id||'')===String(id||''));
    return row ? pitCanonWhole(row.quantity) : null;
  }

  function pitCanonResourceQuantity(documentValue,id,kind='item') {
    const roots=[documentValue,documentValue?.player].filter(Boolean);
    for(const root of roots){
      const rows=kind==='currency' ? (root?.currencies||[]) : (root?.items||[]);
      const field=kind==='currency' ? 'currency_id' : 'item_id';
      const row=(Array.isArray(rows)?rows:[]).find(x=>String(x?.[field]||x?.id||'')===String(id||''));
      if(row)return pitCanonWhole(row?.quantity ?? row?.value);
    }
    return 0;
  }

  function pitCanonCurrencyMax(documentValue,id) {
    const roots=[documentValue,documentValue?.player].filter(Boolean);
    for(const root of roots){
      const row=(root?.currencies||[]).find(x=>String(x?.currency_id||x?.id||'')===String(id||''));
      const value=Number(row?.max_value ?? row?.maxValue ?? row?.limit);
      if(Number.isFinite(value)&&value>0)return Math.trunc(value);
    }
    return 0;
  }

  function pitCanonPremiumUnitPrice(index) {
    if(index<=3)return 25;
    if(index<=6)return 50;
    if(index<=8)return 100;
    if(index<=10)return 125;
    if(index===11)return 150;
    if(index<=13)return 175;
    return 200;
  }

  function pitCanonPremiumCumulative(count) {
    let total=0;
    for(let index=1;index<=pitCanonWhole(count);index++)total+=pitCanonPremiumUnitPrice(index);
    return total;
  }

  function pitCanonPremiumPairs(state,available) {
    const result=[];
    for(const row of state?.pass_costs||[]){
      if(row?.is_prem!==true)continue;
      const mass=pitCanonWhole(row.mass_multiplier);
      const deficit=Math.max(0,mass-pitCanonWhole(available));
      const cost=pitCanonCostQuantity(row?.cost,'cur_prem','currency');
      if(deficit>0&&cost!==null)result.push({deficit,cost,mass});
    }
    return result;
  }

  function pitCanonPremiumOffset(state,available) {
    const pairs=pitCanonPremiumPairs(state,available);
    if(!pairs.length)return 0;
    for(let offset=0;offset<=250;offset++){
      if(pairs.every(pair=>pitCanonPremiumCumulative(offset+pair.deficit)-pitCanonPremiumCumulative(offset)===pair.cost))return offset;
    }
    return null;
  }

  function pitCanonPremiumCost(state,available,deficit) {
    deficit=pitCanonWhole(deficit);
    if(deficit<=0)return {cost:0,verified:true};
    const offset=pitCanonPremiumOffset(state,available);
    if(offset!==null)return {cost:pitCanonPremiumCumulative(offset+deficit)-pitCanonPremiumCumulative(offset),verified:true};
    const direct=pitCanonPremiumPairs(state,available).find(pair=>pair.deficit===deficit);
    return {cost:direct?.cost??0,verified:!!direct};
  }

  function pitCanonDecompose(total) {
    let remaining=pitCanonWhole(total),result=[];
    for(const chunk of HK_PITS_CANON_BATCHES){
      while(remaining>=chunk){result.push(chunk);remaining-=chunk;}
    }
    return result;
  }

  function pitCanonDirectPassCost(state,mass,payment) {
  ...
  const pitCanonDecisionBudget={remember:false,remaining:0};
  ...
  function pitCanonPassivePitPassRate(data=playerDocument){
  ...
  function pitCanonPlans(state,available,itemPasses) {
    ...
    const addAuto=(kind,total,fallback=null)=>{
      total=pitCanonWhole(total);
      if(!total)return;
      const chunks=pitCanonDecompose(total);
      const sig=chunks.join('+');
      if(!chunks.length||seen.has('A:'+sig))return;
      const extra=Math.max(0,total-available);
      const premium=pitCanonPremiumCost(state,available,extra);
      if(extra>0&&!premium.verified&&fallback===null)return;
      seen.add('A:'+sig);
      plans.push({id:`${kind}-${sig}`,kind,total,chunks,paymentMode:'AUTO',crystalCost:premium.verified?premium.cost:pitCanonWhole(fallback),itemCost:0,affordable:true});
    };
    if(available>0)addAuto('exact',available);
    if(available===0){
      addAuto('direct',1,pitCanonPremiumCumulative(1));
      addAuto('rounded',5,pitCanonPremiumCumulative(5));
    }else{
      const rounded=Math.ceil(available/5)*5;
      if(rounded>available)addAuto('rounded',rounded,pitCanonPremiumCumulative(rounded-available));
      const standards=[1,5,10,20,50];
      let direct=standards.find(value=>value>=available);
      if(direct===available)direct=standards.find(value=>value>available);
      if(direct)addAuto('direct',direct,pitCanonPremiumCumulative(Math.max(0,direct-available)));
      else if(available>50){
        const combined=Math.ceil((available+1)/10)*10;
        addAuto('direct',combined,pitCanonPremiumCumulative(Math.max(0,combined-available)));
      }
    }
    for(const batch of HK_PITS_CANON_BATCHES){
      const cost=pitCanonDirectPassCost(state,batch,'ITEM');
      if(cost===null)continue;
      plans.push({id:`item-${batch}`,kind:'item',total:batch,chunks:[batch],paymentMode:'ITEM',crystalCost:0,itemCost:cost,affordable:itemPasses>=cost});
    }
    return plans;
  }

  function pitCanonTargetChoices(state) {
  ...
  function pitCanonRowRunnable(row) {
    ...
    if(row.planId==='reward-0'&&pitCanonWhole(row.rewardTargetMin)>0){
  ...
  function pitCanonDefinitionName(def) { return tr(def.textKey); }

  function pitCanonBuildRows(documentValue=playerDocument) {
    ...
    return HK_PITS_CANON_DEFINITIONS.map(def=>{
      ...
      let selectedPlan=plans.find(row=>row.id===String(old.planId||''))||plans.find(row=>row.kind==='exact')||plans.find(row=>row.affordable)||plans[0]||null;
  ...
  function pitCanonPlanLabel(plan) {
    ...
    if(plan.kind==='exact')return `${either('Точно по доступным пропускам','Exact available passes')}: ${batch}`;
    if(plan.kind==='rounded')return `${either('Округлить вверх','Round up')}: ${batch}${plan.crystalCost?` · 💎 ${plan.crystalCost}`:''}`;
    if(plan.kind==='item')return `${either('Пропуск Ямы','Pit Pass')}: ${batch} · 🎟 ${plan.itemCost}`;
    return `${either('Прямой пакет','Direct batch')}: ${batch}${plan.crystalCost?` · 💎 ${plan.crystalCost}`:''}`;
  }

  function pitCanonSaveDom() {
    const box=root?.querySelector('#hk-pits-content');
    if(!box)return;
  ...
  function pitCanonRewardEntryHtml(entry){
  ...
  function pitCanonRender() {
    ...
    box.innerHTML=`
      ...
      <div class="hk-pits-toolbar"><label class="hk-pit-canon-global"><input id="hk-pits-autofinish-all" type="checkbox" ${allAutofinish?'checked':''}><span>${either('Автозавершение всех Ям','Auto-finish all Pits')}</span></label></div>
      <div class="hk-pits-cards">${rows.map(row=>{
        ...
        const sharedForecast=pitCanonRewardSharedForecastHtml(row,rows);
    ...
    box.querySelectorAll('[data-pit-reward-open]').forEach(button=>button.onclick=()=>pitCanonOpenRewardPicker(button.dataset.pitRewardOpen));
    box.querySelector('#hk-pits-start').onclick=runPitsCanonical;
    renderPitForecast();
  }

  function pitCanonReadRunConfigs() {
    pitCanonSaveDom();
    const rows=pitCanonBuildRows();
  ...
  function pitCanonPreflightExpectedBoxReturns(pool,useBoxes){
    if(!useBoxes)return;
  ...
  async function pitCanonRunOne(config,progress) {
    ...
    for(let index=0;index<config.steps.length;index++){
      ...
      if(resuming){
        ...
        if(payment==='ITEM'&&itemCost===null)throw new Error(`${pitCanonDefinitionName(def)}: ${either('стоимость Пропуска Ямы изменилась','Pit Pass cost changed')}`);
        if(spentCrystals+pitCanonWhole(premCost)>config.crystalBudget)throw new Error(`${pitCanonDefinitionName(def)}: ${either('стоимость в кристаллах выросла','crystal cost increased')}`);
        if(spentItems+pitCanonWhole(itemCost)>config.itemPassBudget)throw new Error(`${pitCanonDefinitionName(def)}: ${either('стоимость в Пропусках Ямы выросла','Pit Pass cost increased')}`);
  ...
  const bossCanonState={
    ...
    areaProgress:{passId:'',score:0,level:0,levels:[],rewardsByLevel:{},endTime:0},
    area:{enabled:false,bossLevel:'',planId:'',maxRestoration:0,autofinish:true,targetLevel:0,targetStrategy:'last_day',dailyBaseRuns:null},
  ...
  function bossCanonStored(){
    ...
    for(const kind of ['area','regional']){
      ...
      if(kind==='area'){bossCanonState.area.targetLevel=bossCanonWhole(source.targetLevel);bossCanonState.area.targetStrategy=bossCanonTargetStrategy(source.targetStrategy);bossCanonState.area.dailyBaseRuns=source.dailyBaseRuns===null||source.dailyBaseRuns===undefined?null:bossCanonWhole(source.dailyBaseRuns);}
    }
  ...
  function bossCanonSave(){
    save({bossCanonSettings:{
      area:{enabled:!!bossCanonState.area.enabled,bossLevel:String(bossCanonState.area.bossLevel||''),planId:String(bossCanonState.area.planId||''),maxRestoration:bossCanonWhole(bossCanonState.area.maxRestoration),autofinish:!!bossCanonState.area.autofinish,targetLevel:bossCanonWhole(bossCanonState.area.targetLevel),targetStrategy:bossCanonTargetStrategy(bossCanonState.area.targetStrategy),dailyBaseRuns:bossCanonState.area.dailyBaseRuns===null?null:bossCanonWhole(bossCanonState.area.dailyBaseRuns)},
  ...
  function bossCanonActivePlan(kind){
    if(kind==='area'){
      ...
      const mass=Math.max(1,bossCanonWhole(active?.mass_multiplier));return{id:'active',total:1,activeMass:mass,paymentMode:'ACTIVE',steps:[{chunk:mass,payment:'ACTIVE'}],crystalCost:0,itemCost:0,freePasses:0,paidPasses:0,affordable:true};
    }
    const status=bossCanonRegionalStatus(); if(status?.is_finished!==false)return null;
    const mass=Math.max(1,bossCanonWhole(bossCanonState.regionalBattleState?.mass_multiplier||status?.mass_multiplier||1));return{id:'active',total:1,activeMass:mass,paymentMode:'ACTIVE',steps:[{chunk:mass,payment:'ACTIVE'}],crystalCost:0,itemCost:0,freePasses:0,paidPasses:0,affordable:true};
  }
  function bossCanonTargetStrategy(value){return ['upfront','gradual','last_day'].includes(String(value||''))?String(value):'last_day';}
  function bossCanonFutureDailyRefreshCount(endTime,now=Date.now()){
    const end=Number(endTime)||0;if(end<=now)return 0;const day=86400000,resetOffset=12*60*60*1000;let next=(Math.floor(now/day)*day)+resetOffset;if(next<=now)next+=day;if(next>=end)return 0;return Math.floor((end-1-next)/day)+1;
  }
  function bossCanonTargetRequestedNow(required,strategy,{completed=0,gradualDivisor=1,strategyActiveNow=true}={}){
    const normalized=bossCanonTargetStrategy(strategy),needed=bossCanonWhole(required),done=bossCanonWhole(completed),divisor=Math.max(1,bossCanonWhole(gradualDivisor));if(normalized==='upfront')return needed;if(normalized==='gradual'&&needed>0)return Math.max(0,Math.ceil((needed+done)/divisor)-done);if(normalized==='last_day'&&strategyActiveNow)return needed;return 0;
  }
  function bossCanonDailyBaseRuns(documentValue=playerDocument){
    if(bossCanonState.area.dailyBaseRuns!==null&&bossCanonState.area.dailyBaseRuns!==undefined)return bossCanonWhole(bossCanonState.area.dailyBaseRuns);
    const row=(documentValue?.bonuses?.max_currency||[]).find(x=>String(x?.id||'')===HK_BOSS_AREA_PASS_CURRENCY_ID)||{},vip=documentValue?.player?.vip_status_applied?bossCanonWhole(row?.vip):0,value=bossCanonWhole(row?.initial)+bossCanonWhole(row?.bonus)+vip;return value||Math.max(1,bossCanonFreePasses('area',documentValue));
  }
  function bossCanonPassiveTicketsPerHour(documentValue=playerDocument){
    const row=(documentValue?.bonuses?.passive_income_item||[]).find(x=>String(x?.id||'')===HK_BOSS_AREA_TOKEN_ITEM_ID)||{},vip=documentValue?.player?.vip_status_applied?Number(row?.vip||0):0,value=Number(row?.bonus||0)+Number(row?.bonus_building||0)+vip;return Number.isFinite(value)&&value>0?value:0;
  }
  function bossCanonBuildRewards(staticPass){
    const running={},byLevel={};for(const row of [...(staticPass?.levels||[])].sort((a,b)=>Number(a.level)-Number(b.level))){for(const [lineId,line] of Object.entries(row?.lines_level||{})){let key='';if(lineId.endsWith('_line_free'))key='free';else if(lineId.endsWith('_line_paid_01'))key='paid1';else if(lineId.endsWith('_line_paid_02'))key='paid2';else continue;for(const group of ['items','currencies'])for(const reward of line?.reward_view?.[group]||[]){const id=String(reward?.id||''),count=bossCanonWhole(reward?.count);if(!id||!count)continue;if(!running[id])running[id]={free:0,paid1:0,paid2:0,total:0};running[id][key]+=count;running[id].total+=count;}}byLevel[bossCanonWhole(row?.level)]=JSON.parse(JSON.stringify(running));}return byLevel;
  }
  function bossCanonPassLevel(levels,score){let level=0;for(const row of levels||[]){const l=bossCanonWhole(row?.level),needed=bossCanonWhole(row?.score);if(l>0&&needed<=bossCanonWhole(score))level=Math.max(level,l);}return level;}
  async function bossCanonLoadAreaProgress(clientConfig=null){
    try{
      const cfg=clientConfig||await apiJson('/client_config','GET');const ids=[...new Set((((cfg?.points||{}).battle_pass_score)||[]).map(x=>x?.id).filter(id=>typeof id==='string'&&id.startsWith('bp_personal_area_boss')))].sort().reverse();let active=null;
      for(const id of ids){try{const data=await apiJson('/battlepass?battle_pass_type='+encodeURIComponent(id)+'&status=ACTUAL','GET');const bp=data?.player_battle_pass||{};if(Number(bp?.start_timer??1)<=0&&Number(bp?.end_exp_timer??-1)>0){const serverNow=Number(data?.timestamp||Date.now()),endTime=Math.floor((serverNow+Number(bp.end_exp_timer||0))/1000)*1000;active={bp,endTime};break;}}catch(_){}}
      if(!active)throw new Error('no active Area Boss pass');const staticPass=await growthStaticJson('battlepass/list/'+encodeURIComponent(active.bp.id));const levels=Array.isArray(staticPass?.levels)?staticPass.levels:[];bossCanonState.areaProgress={passId:String(active.bp.id||''),score:bossCanonWhole(active.bp.score),level:bossCanonPassLevel(levels,active.bp.score),levels,rewardsByLevel:bossCanonBuildRewards(staticPass),endTime:Math.max(0,Number(active.endTime)||0)};return bossCanonState.areaProgress;
    }catch(_){bossCanonState.areaProgress={passId:'',score:0,level:0,levels:[],rewardsByLevel:{},endTime:0};return bossCanonState.areaProgress;}
  }
  function bossCanonTargetLevelRow(level){const wanted=bossCanonWhole(level);if(!HK_BOSS_AREA_TARGET_LEVELS.includes(wanted))return null;const row=(bossCanonState.areaProgress?.levels||[]).find(x=>bossCanonWhole(x?.level)===wanted),score=bossCanonWhole(row?.score);return score>0?{level:wanted,score}:null;}
  function bossCanonTargetLevels(){return HK_BOSS_AREA_TARGET_LEVELS.map(bossCanonTargetLevelRow).filter(Boolean);}
  function bossCanonTargetLevelForScore(score){return bossCanonPassLevel(bossCanonState.areaProgress?.levels||[],score);}
  function bossCanonAreaItemBatches(documentValue=playerDocument){
    return (documentValue?.boss_battle?.pass_costs||[]).map(row=>{if(row?.is_prem===true)return null;const batch=bossCanonWhole(row?.mass_multiplier),cost=bossCanonExclusiveCost(row,'ITEM',HK_BOSS_AREA_TOKEN_ITEM_ID);return batch>0&&cost!==null?{batch,cost}:null;}).filter(Boolean).sort((a,b)=>b.batch-a.batch);
  }
  function bossCanonExpectedBoxReturn(pool,rate=0.024){
    const available=bossCanonWhole(pool.boxes);if(available<100)return false;const opened=Math.min(1000,available);pool.boxes=available-opened;pool.openedBoxes=bossCanonWhole(pool.openedBoxes)+opened;const expected=Math.max(bossCanonWhole(pool.generatedPasses),Math.ceil(pool.openedBoxes*Math.max(0,Number(rate)||0))),gained=Math.max(0,expected-bossCanonWhole(pool.generatedPasses));pool.generatedPasses=expected;pool.passes=bossCanonWhole(pool.passes)+gained;return true;
  }
  function bossCanonSimulateTicketEconomy({passes=0,boxes=0,required=0,boxesPerRun=0,supported=[]}={}){
    const pool={passes:bossCanonWhole(passes),boxes:bossCanonWhole(boxes),openedBoxes:0,generatedPasses:0},groups=[];bossCanonExpectedBoxReturn(pool);let remaining=bossCanonWhole(required);while(remaining>0){const option=supported.find(x=>x.batch<=remaining&&x.cost<=pool.passes);if(!option)return{...pool,groups,completed:bossCanonWhole(required)-remaining,remaining,affordable:false};const last=groups.at(-1);if(last&&last.batch===option.batch&&last.costEach===option.cost)last.count++;else groups.push({batch:option.batch,count:1,costEach:option.cost});pool.passes-=option.cost;remaining-=option.batch;pool.boxes+=bossCanonWhole(boxesPerRun)*option.batch;bossCanonExpectedBoxReturn(pool);}return{...pool,groups,completed:bossCanonWhole(required),remaining:0,affordable:true};
  }
  function bossCanonExpandGroups(groups){const out=[];for(const g of groups||[])for(let i=0;i<bossCanonWhole(g?.count);i++)out.push(bossCanonWhole(g?.batch));return out.filter(Boolean);}
  function bossCanonTargetPlan(level,{currentScore=null,completedToday=0,basePlan=null}={}){
    const target=bossCanonTargetLevelRow(level),boss=bossCanonSelectedArea(),endTime=Number(bossCanonState.areaProgress?.endTime)||0,now=Date.now(),score=currentScore===null?bossCanonWhole(bossCanonState.areaProgress?.score):bossCanonWhole(currentScore),strategy=bossCanonTargetStrategy(bossCanonState.area.targetStrategy),pointsPer=bossCanonWhole(boss?.points),boxesPer=bossCanonWhole(boss?.boxes);
    const result={level:bossCanonWhole(level),targetScore:bossCanonWhole(target?.score),currentScore:score,known:false,valid:false,reachable:false,alreadyReached:false,pointsPerAttempt:pointsPer,boxesPerAttempt:boxesPer,todayBaseAttempts:0,futureDailyRefreshes:0,futureFreeAttempts:0,futurePassiveTickets:0,requiredExtraAttempts:0,requestedNow:0,scheduledNow:0,scheduledGroups:[],projectedAfterToday:score,projectedFinal:score,maxProjected:score,reachLevel:bossCanonTargetLevelForScore(score),maxReachLevel:bossCanonTargetLevelForScore(score),remainingAfterBase:Math.max(0,bossCanonWhole(target?.score)-score),strategy,strategyActiveNow:false,gradualDivisor:1};
    if(!target||!boss||endTime<=now||pointsPer<=0)return result;const plan=basePlan||bossCanonSelectedPlan('area'),todayAttempts=(plan?.kind==='reward-zero'?0:(plan?.steps||[]).reduce((sum,x)=>sum+bossCanonWhole(x?.chunk),0)),todayItemCost=plan?.paymentMode==='ITEM'?bossCanonWhole(plan.itemCost):0,currentTickets=bossCanonResource(playerDocument,HK_BOSS_AREA_TOKEN_ITEM_ID,'item'),currentBoxes=bossCanonResource(playerDocument,HK_BOSS_AREA_LOOTBOX_ITEM_ID,'item'),ticketsAfterBase=Math.max(0,currentTickets-todayItemCost),refreshes=bossCanonFutureDailyRefreshCount(endTime,now),futureFree=refreshes*bossCanonDailyBaseRuns(),remainingMs=Math.max(0,endTime-now),futurePassive=Math.max(0,Math.floor(bossCanonPassiveTicketsPerHour()*(remainingMs/3600000))),afterToday=score+todayAttempts*pointsPer,afterDaily=afterToday+futureFree*pointsPer,remaining=Math.max(0,result.targetScore-afterDaily),required=remaining>0?Math.ceil(remaining/pointsPer):0,divisor=Math.max(1,Math.ceil(remainingMs/86400000)),activeNow=strategy!=='last_day'||remainingMs<86400000,requested=bossCanonTargetRequestedNow(required,strategy,{completed:completedToday,gradualDivisor:divisor,strategyActiveNow:activeNow}),batches=bossCanonAreaItemBatches(),baseBoxes=currentBoxes+(todayAttempts+futureFree)*boxesPer,forecast=bossCanonSimulateTicketEconomy({passes:ticketsAfterBase+futurePassive,boxes:baseBoxes,required,boxesPerRun:boxesPer,supported:batches}),todayEconomy=bossCanonSimulateTicketEconomy({passes:ticketsAfterBase,boxes:currentBoxes+todayAttempts*boxesPer,required:requested,boxesPerRun:boxesPer,supported:batches}),maxEconomy=bossCanonSimulateTicketEconomy({passes:ticketsAfterBase+futurePassive,boxes:baseBoxes,required:100000,boxesPerRun:boxesPer,supported:batches}),scheduled=todayEconomy.completed,projectedAfterToday=afterToday+scheduled*pointsPer,projectedFinal=afterDaily+required*pointsPer,maxProjected=afterDaily+maxEconomy.completed*pointsPer;
    Object.assign(result,{known:true,todayBaseAttempts:todayAttempts,futureDailyRefreshes:refreshes,futureFreeAttempts:futureFree,futurePassiveTickets:futurePassive,requiredExtraAttempts:required,requestedNow:requested,scheduledNow:scheduled,scheduledGroups:bossCanonExpandGroups(todayEconomy.groups),projectedAfterToday,projectedFinal,maxProjected,reachLevel:bossCanonTargetLevelForScore(projectedFinal),maxReachLevel:bossCanonTargetLevelForScore(maxProjected),remainingAfterBase:remaining,strategyActiveNow:activeNow,gradualDivisor:divisor,alreadyReached:Math.max(0,result.targetScore-score)===0});result.reachable=projectedFinal>=result.targetScore&&forecast.affordable;result.valid=result.reachable&&bossCanonState.area.autofinish;return result;
  }
  function bossCanonRewardOnlyPlan(){return{id:'reward-0',kind:'reward-zero',total:0,paymentMode:'NONE',steps:[],crystalCost:0,itemCost:0,freePasses:0,paidPasses:0,affordable:true};}
  function bossCanonBuildPlans(kind){
    ...
    if(kind==='area'&&bossCanonWhole(bossCanonState.area.targetLevel)>0)plans.unshift(bossCanonRewardOnlyPlan());
    return plans;
  }
  function bossCanonPlanLabel(kind,plan){
    if(!plan)return''; if(plan.kind==='reward-zero')return either('Только цель награды — 0 базовых запусков','Reward target only — 0 base runs'); if(plan.paymentMode==='ACTIVE')return `${either('Продолжить активный бой','Continue active battle')} — ×${plan.total}`;
  ...
  function bossCanonClamp(){
    ...
    if(bossCanonWhole(bossCanonState.area.targetLevel)>0&&!bossCanonTargetLevelRow(bossCanonState.area.targetLevel))bossCanonState.area.targetLevel=0;
  ...
  async function bossCanonLoadData(){
    ...
    const [view,preview,cfg]=await Promise.all([apiJson('/bosses/view','GET'),apiJson('/player/area_boss_battle_preview','GET').catch(()=>null),apiJson('/client_config','GET').catch(()=>null)]);
    bossCanonState.view=view;bossCanonState.preview=preview;bossCanonState.areaBosses=bossCanonBuildAreaBosses(view,preview);bossCanonState.regionalBosses=bossCanonBuildRegionalBosses(view,playerDocument);await bossCanonLoadAreaProgress(cfg);
  ...
  async function bossCanonOpenAreaLootboxes(){
    let opened=false;while(true){playerDocument=await hkAuthoritativePlayerRead('bosses:area:boxes-pre');const boxes=bossCanonResource(playerDocument,HK_BOSS_AREA_LOOTBOX_ITEM_ID,'item');if(boxes<100)break;const quantity=Math.min(1000,boxes),before=bossCanonResource(playerDocument,HK_BOSS_AREA_TOKEN_ITEM_ID,'item');await apiJson('/player/boxes/open?item_id='+encodeURIComponent(HK_BOSS_AREA_LOOTBOX_ITEM_ID)+'&quantity='+quantity,'POST');playerDocument=await hkAuthoritativePlayerRead('bosses:area:boxes-after');const after=bossCanonResource(playerDocument,HK_BOSS_AREA_TOKEN_ITEM_ID,'item');log(`${either('Коробки дани','Tribute Boxes')}: ${quantity} · 🎟 +${Math.max(0,after-before)}`,'info');opened=true;break;}return opened;
  }
  async function bossCanonRunAreaItemChunk({bossLevel,chunk,maxRestoration,name,progress}){
    await hkRunner.waitIfPaused();if(hkRunner.signal?.aborted)throw new DOMException('Aborted','AbortError');playerDocument=await hkAuthoritativePlayerRead('bosses:area:target-pre');const option=bossCanonAreaItemBatches().find(x=>x.batch===bossCanonWhole(chunk));if(!option)throw new Error(either('Стоимость турнирного прохода изменилась','Tournament pass cost changed'));const have=bossCanonResource(playerDocument,HK_BOSS_AREA_TOKEN_ITEM_ID,'item');if(have<option.cost)return false;hkRunner.setStep(`${name} · ${either('цель награды','reward target')} ×${chunk}`,progress.done,progress.total);log(`▶ ${name} · ${either('турнирный запуск','tournament run')} ×${chunk} · 🎟 ${option.cost}`,'info');await apiJson('/bosses/pass','POST',{boss_level:Number(bossLevel),mass_multiplier:bossCanonWhole(chunk),payment_type:'ITEM'});playerDocument=await hkAuthoritativePlayerRead('bosses:area:target-after-pass');let completed=true;if(playerDocument?.boss_battle?.is_finish===false){const finish=await bossCanonFinishAreaAttempt({bossLevel,mass:chunk,maxRestoration,name});completed=finish.finished;}progress.done+=bossCanonWhole(chunk);if(completed)log(`✓ ${name} · ${either('турнирный запуск завершён','tournament run completed')} ×${chunk}`,'ok');return completed;
  }
  async function bossCanonRunAreaTargetContinuation(config,progress,name){
    if(bossCanonWhole(config?.targetLevel)<=0)return true;let completedToday=0;await bossCanonLoadAreaProgress();await bossCanonOpenAreaLootboxes();
    while(true){await bossCanonLoadAreaProgress();const target=bossCanonTargetPlan(config.targetLevel,{completedToday,basePlan:bossCanonRewardOnlyPlan()}),score=bossCanonWhole(bossCanonState.areaProgress.score);if(score>=bossCanonWhole(config.targetScore)){log(`✓ ${name} · ${either('цель награды достигнута','reward target reached')}: ${score}/${config.targetScore}`,'ok');return true;}const wanted=bossCanonWhole(target.requestedNow);if(wanted<=0){log(`${name} · ${either('цель остаётся по графику будущих запусков','target remains on schedule with future runs')}`,'ok');return true;}const remaining=Math.max(0,wanted-completedToday);if(!remaining)return true;playerDocument=await hkAuthoritativePlayerRead('bosses:area:target-batch');let have=bossCanonResource(playerDocument,HK_BOSS_AREA_TOKEN_ITEM_ID,'item'),options=bossCanonAreaItemBatches().filter(x=>x.batch<=remaining&&x.cost<=have);if(!options.length){await bossCanonOpenAreaLootboxes();playerDocument=await hkAuthoritativePlayerRead('bosses:area:target-batch-reread');have=bossCanonResource(playerDocument,HK_BOSS_AREA_TOKEN_ITEM_ID,'item');options=bossCanonAreaItemBatches().filter(x=>x.batch<=remaining&&x.cost<=have);}const option=options[0];if(!option){log(`${name} · ${either('турнирный план безопасно остановлен: не хватает приглашений','reward plan stopped safely: not enough invitations')}`,'warn');return false;}const next=completedToday+option.batch;if(next>completedToday&&progress.done+option.batch>progress.total)progress.total=progress.done+option.batch;const ok=await bossCanonRunAreaItemChunk({bossLevel:config.bossLevel,chunk:option.batch,maxRestoration:config.maxRestoration,name,progress});if(!ok)return false;completedToday=next;await bossCanonOpenAreaLootboxes();await bossCanonLoadAreaProgress();const recalculated=bossCanonTargetPlan(config.targetLevel,{completedToday,basePlan:bossCanonRewardOnlyPlan()});log(`${name} · ${either('цель пересчитана','target recalculated')}: ${bossCanonState.areaProgress.score}/${config.targetScore} · ${either('ещё сейчас','still now')} ${recalculated.requestedNow}`,'info');}
  }
  ...
  async function bossCanonRunArea(config,progress){
    ...
    if(config.paymentMode==='ACTIVE'){
      if(active?.is_finish!==false||bossCanonWhole(active.level)!==bossCanonWhole(config.bossLevel))throw new Error(either('План босса изменился','Boss plan changed'));const mass=Math.max(1,bossCanonWhole(active.mass_multiplier));log(`▶ ${name} · ${either('продолжение активного боя','continuing active battle')} ×${mass}`,'info');if(!config.autofinish)return false;const result=await bossCanonFinishAreaAttempt({bossLevel:config.bossLevel,mass,maxRestoration:config.maxRestoration,name});progress.done+=1;if(!result.finished)return false;return await bossCanonRunAreaTargetContinuation(config,progress,name);
    }
    ...
    if(livePlan.kind==='reward-zero'||livePlan.paymentMode==='NONE')return await bossCanonRunAreaTargetContinuation(config,progress,name);
  ...
  function bossCanonReadRunConfigs(){
    ...
    for(const kind of ['area','regional']){const row=bossCanonState[kind],selected=kind==='area'?bossCanonSelectedArea():bossCanonSelectedRegional(),plan=bossCanonSelectedPlan(kind);if(!row.enabled||!selected||!plan||plan.affordable===false)continue;const target=kind==='area'&&bossCanonWhole(row.targetLevel)>0?bossCanonTargetPlan(row.targetLevel,{basePlan:plan}):null;if(target&&!target.valid)continue;result.push({kind,bossLevel:kind==='area'?Number(row.bossLevel):undefined,bossId:kind==='regional'?String(row.bossId):undefined,planId:plan.id,total:plan.total,executionTotal:bossCanonWhole(plan.total)+bossCanonWhole(target?.scheduledNow),steps:plan.steps.map(step=>({...step})),paymentMode:plan.paymentMode,crystalBudget:plan.crystalCost,itemPassBudget:plan.itemCost,freePasses:plan.freePasses,paidPasses:plan.paidPasses,maxRestoration:Math.min(bossCanonWhole(row.maxRestoration),bossCanonResource(playerDocument,HK_BOSS_RESTORATION_ITEM_ID,'item')),autofinish:!!row.autofinish,passCost:kind==='regional'?bossCanonRetryCost('regional'):null,targetLevel:bossCanonWhole(target?.level),targetScore:bossCanonWhole(target?.targetScore),targetStrategy:target?target.strategy:'last_day',targetRequestedNow:bossCanonWhole(target?.requestedNow),targetScheduledNow:bossCanonWhole(target?.scheduledNow),targetRequiredExtra:bossCanonWhole(target?.requiredExtraAttempts),targetProjectedFinal:bossCanonWhole(target?.projectedFinal)});}
    return result;
  }
  ...
  function bossCanonTargetUi(){
    const row=bossCanonState.area,levels=bossCanonTargetLevels(),level=bossCanonWhole(row.targetLevel),plan=level?bossCanonTargetPlan(level):null,current=bossCanonWhole(bossCanonState.areaProgress?.score),options=['<option value="0">'+either('Без цели награды','No reward target')+'</option>',...levels.map(x=>`<option value="${x.level}"${x.level===level?' selected':''}>${either('Ур.','Lv')} ${x.level} · ${x.score.toLocaleString(locale())} BP XP</option>`)].join(''),strategy=bossCanonTargetStrategy(row.targetStrategy),strategyOptions=[['upfront',either('Сразу','Upfront')],['gradual',either('Постепенно','Gradual')],['last_day',either('В последний день','Last day')]].map(([v,t])=>`<option value="${v}"${strategy===v?' selected':''}>${t}</option>`).join('');
    const stats=plan?`<div class="hk-boss-target-stats"><span>${either('Сейчас','Current')}: <b>${current.toLocaleString(locale())}</b></span><span>${either('За попытку','Per attempt')}: <b>${plan.pointsPerAttempt.toLocaleString(locale())}</b></span><span>${either('Доп. сейчас','Extra now')}: <b>${plan.scheduledNow}</b></span><span>${either('Прогноз','Projected')}: <b>${plan.projectedFinal.toLocaleString(locale())}</b></span><span>${either('Достижимый уровень','Reach level')}: <b>${plan.maxReachLevel||'—'}</b></span><span>${either('Статус','Status')}: <b>${plan.valid?either('достижимо','reachable'):either('не подтверждено','not confirmed')}</b></span></div>`:'';
    return `<details class="hk-pit-reward-block" ${level?'open':''}><summary><span>${either('Турнирная цель Area Boss','Area Boss tournament target')}</span><b class="${level?'active':''}">${level?`${either('Ур.','Lv')} ${level}`:either('выкл.','off')}</b></summary><div class="hk-pit-reward-body"><div class="hk-pit-reward-controls"><label><span>${either('Целевой уровень','Target level')}</span><select data-boss-target-level="area">${options}</select></label><label><span>${either('Стратегия','Strategy')}</span><select data-boss-target-strategy="area">${strategyOptions}</select></label><label><span>${either('Базовых попыток в день','Daily base runs')}</span><input data-boss-daily-base-runs="area" type="number" min="0" value="${bossCanonDailyBaseRuns()}"></label></div>${stats}<small class="hk-pit-reward-note">${either('После каждого завершённого раунда очки и оставшийся план пересчитываются по live battle pass.','After every completed round, live battle-pass score and remaining plan are recalculated.')}</small></div></details>`;
  }
  ...
  function renderBosses(){
    ...
    box.querySelectorAll('[data-boss-target-level],[data-boss-target-strategy],[data-boss-daily-base-runs]').forEach(control=>control.onchange=()=>{if(control.dataset.bossTargetLevel!==undefined){bossCanonState.area.targetLevel=bossCanonWhole(control.value);if(bossCanonState.area.targetLevel>0&&bossCanonState.area.planId==='')bossCanonState.area.planId='reward-0';}else if(control.dataset.bossTargetStrategy!==undefined)bossCanonState.area.targetStrategy=bossCanonTargetStrategy(control.value);else bossCanonState.area.dailyBaseRuns=bossCanonWhole(control.value);bossCanonClamp();bossCanonSave();renderBosses();});
  }
  ...
  const HK_CLAN_SHOP_PLAYER_ME_FACTS_REV='clan-shop-player-me-facts-20260922-r2';
  ...
  function dailyCanonEventRegularRows() {
  ...
  function dailyActionRows() {
    ...
    dailyCanonEventRegularRows().forEach(x=>push(x,'event'));
    actions.push({id:'claimLeaderboardRewards',label:either('Собрать награды рейтинга: Ямы / Боссы / Крысы','Claim leaderboard rewards: Pits / Bosses / Rats'),kind:'claimLeaderboardRewards',uiGroup:'rewards',free:true,available:true});
    actions.push({id:'claimAreaBossBattlePass',label:either('Собрать награды Battle Pass боссов','Claim Area Boss Battle Pass rewards'),kind:'claimAreaBossBattlePass',uiGroup:'rewards',free:true,available:true});
  ...
  const HK_PITS_PREFLIGHT_REV = 'pits-aggregate-preflight-20260920-r19';
  const HK_PITS_REWARD_PICKER_REV = 'pits-reward-picker-20260920-r20';
  ...
  function mapBuildingAreaCacheKey() {
    return String(playerIdentity((hkStateStore.snapshot||playerDocument)?.player||{}) || '');
  }

  function mapHydrateBuildingAreas() {
    const playerKey=mapBuildingAreaCacheKey();
    if(!playerKey)return 0;
    const rootCache=load().mapBuildingAreasByPlayer;
    const rows=rootCache&&typeof rootCache==='object'&&rootCache[playerKey]&&typeof rootCache[playerKey]==='object'?rootCache[playerKey]:{};
    let count=0;
    for(const [buildingId,areaId] of Object.entries(rows)){
      if(!buildingId||!areaId)continue;
      if(!mapBuildingAreas.has(String(buildingId)))mapBuildingAreas.set(String(buildingId),String(areaId));
      count+=1;
    }
    return count;
  }

  function mapPersistOwnedBuildingAreas() {
    const playerKey=mapBuildingAreaCacheKey();
    if(!playerKey)return false;
    const ownedIds=new Set(((hkStateStore.snapshot||playerDocument)?.areas?.areas||[])
      .map(row=>String(row?.gamearea_id||row?.area_id||'')).filter(Boolean));
    const rows={};
    for(const [buildingId,areaId] of mapBuildingAreas.entries()){
      if(buildingId&&ownedIds.has(String(areaId)))rows[String(buildingId)]=String(areaId);
    }
    const stored=load();
    const rootCache=stored.mapBuildingAreasByPlayer&&typeof stored.mapBuildingAreasByPlayer==='object'?stored.mapBuildingAreasByPlayer:{};
    localStorage.setItem(STORE,JSON.stringify({...stored,mapBuildingAreasByPlayer:{...rootCache,[playerKey]:rows}}));
    return true;
  }

  function mapRememberBuildingArea(buildingId,areaId) {
    const id=String(buildingId||''), area=String(areaId||'');
    if(!id||!area)return false;
    mapBuildingAreas.set(id,area);
    return true;
  }

  async function mapEnsureOwnedBuildingAreaIndex(force=false) {
    mapHydrateBuildingAreas();
    const state=hkStateStore.snapshot||playerDocument||{};
    const owned=(state?.areas?.areas||[]).filter(row=>row?.gamearea_id||row?.area_id);
    const activeIds=new Set((state?.buildings||[]).map(row=>String(row?.id||row?.building_id||'')).filter(Boolean));
    if(!force&&activeIds.size&&[...activeIds].every(id=>mapBuildingAreas.has(id)))return mapBuildingAreas;
    if(mapOwnedBuildingIndexPromise)return mapOwnedBuildingIndexPromise;
    mapOwnedBuildingIndexPromise=(async()=>{
      let cursor=0;
      const worker=async()=>{
        while(cursor<owned.length){
          const row=owned[cursor++];
          const areaId=String(row?.gamearea_id||row?.area_id||'');
          if(!areaId)continue;
          let definitions=[];
          try{definitions=await apiJson(`/game_area/${encodeURIComponent(areaId)}/buildings`,'GET');}catch(_){continue;}
          for(const definition of (Array.isArray(definitions)?definitions:[])){
            const buildingId=String(definition?.building_id||'');
            if(buildingId)mapRememberBuildingArea(buildingId,areaId);
          }
        }
      };
      await Promise.all(Array.from({length:Math.min(4,Math.max(1,owned.length))},()=>worker()));
      mapPersistOwnedBuildingAreas();
      return mapBuildingAreas;
    })().finally(()=>{mapOwnedBuildingIndexPromise=null;});
    return mapOwnedBuildingIndexPromise;
  }

  async function mapResolveBuildingArea(buildingId) {
    const id=String(buildingId||'');
    if(!id)return '';
    mapHydrateBuildingAreas();
    let areaId=String(mapBuildingAreas.get(id)||'');
    if(areaId)return areaId;
    try{await mapEnsureOwnedBuildingAreaIndex(false);}catch(_){}
    areaId=String(mapBuildingAreas.get(id)||'');
    return areaId;
  }

  async function mapSubmitBuildingStudy(buildingId,areaId,roomCount,hasEvents) {
    const id=String(buildingId||''), area=String(areaId||'');
  ...
  async function mapAreaPayload(area, cities = []) {
    ...
    const buildings = (Array.isArray(definitions) ? definitions : []).map(row => {
      ...
      const study=buildingStudyCache.get(id)||state;
      const detectedRooms=study ? crystalRoomCount(study) : null;
  ...
  async function submitOwnedMapAreas(all = true) {
    ...
    mapScanning = true;
    try {
      playerDocument=await hkAuthoritativePlayerRead('maps:scan-start');
      await ensureRecipeMetadata();
      mapHydrateBuildingAreas();
      ...
      const selectedAreaIds=new Set(selected.map(row=>String(row?.gamearea_id||row?.area_id||'')).filter(Boolean));
      ...
      mapPersistOwnedBuildingAreas();
      hkRunner.setStep(either('Считываю уже открытые здания','Reading already active buildings'),selected.length,selected.length);
      const backfill=await mapBackfillActiveBuildingStudies(selectedAreaIds);
      ...
      log(either(
        `Карты отправлены: районов ${selected.length} · активных зданий считано ${backfill.studied}/${backfill.total}${backfill.failed?` · ошибок ${backfill.failed}`:''}`,
        `Maps submitted: ${selected.length} districts · active buildings read ${backfill.studied}/${backfill.total}${backfill.failed?` · errors ${backfill.failed}`:''}`
      ),backfill.failed?'warn':'ok');
  ...
  function businessCatalogOwnedCount(businessId, documentValue = playerDocument) {
  ...
  async function businessAction(action, row, businessId = null, timer = null) {
    ...
    if (action === 'insert') payload.business_id = businessId;
  ...
  function businessCanonicalInspection(action,row,expectedBusinessId='') {
    const state=findSlot(playerDocument,row.buildingId,row.slot);
    const currentId=String(state?.businessId||'');
    const expected=String(expectedBusinessId||'');
    if(action==='remove'){
      return {state,applied:!!state&&!currentId,retryable:!!state&&!!currentId&&(!expected||currentId===expected),reason:either('Слот изменился','Slot changed')};
    }
    if(action==='insert'){
      return {state,applied:!!state&&currentId===expected,retryable:!!state&&!currentId,reason:either('Слот изменился','Slot changed')};
    }
    if(action==='speedUp'){
      const timer=Math.max(0,Number(state?.timer||0));
      const free=Math.max(0,Number(state?.freeSpeedUpTime||0));
      const matches=!!state&&currentId===expected;
      return {state,applied:matches&&(String(state?.status||'').toUpperCase()==='ACTIVE'||timer<=0),retryable:matches&&timer>0&&free>=timer,reason:matches?either('Ожидание ещё не завершено','Not instant'):either('Слот изменился','Slot changed')};
    }
    if(action==='activate'){
      const matches=!!state&&currentId===expected;
      return {state,applied:matches&&String(state?.status||'').toUpperCase()==='ACTIVE',retryable:matches&&Number(state?.timer||0)<=0,reason:either('Слот изменился','Slot changed')};
    }
    return {state,applied:false,retryable:false,reason:either('Неизвестная операция','Unknown operation')};
  }

  async function businessCanonicalMutation(action,row,businessId='',label='',timer=null) {
    const expected=String(businessId||row?.businessId||'');
    const name=label||businessDisplayName(expected);
    const request=()=>businessAction(action,row,action==='insert'?expected:null,action==='speedUp'?timer:null);
    try{
      ...
      try{
        playerDocument=await hkAuthoritativePlayerRead('business-canon-'+action+'-refresh');
      }catch(refreshError){
        recordDiagnostic('business-canon-refresh-failed',{action,buildingId:row.buildingId,slot:row.slot,error:refreshError?.message||String(refreshError)});
        throw firstError;
      }
      const refreshed=businessCanonicalInspection(action,row,expected);
      if(refreshed.applied){
        hkRunner.note(either('Действие уже применено игрой','Game action was already applied'),'ok');
        return {__hk_recovered:true,state:refreshed.state};
      }
      if(!refreshed.retryable){
        hkRunner.note(either('Состояние слота изменилось — повтор запрещён','Slot state changed — retry blocked'),'bad');
        const conflict=new Error(refreshed.reason||either('Слот изменился','Slot changed'));
        conflict.cause=firstError;
        throw conflict;
      }
      hkRunner.note(either('Повторяю действие один раз после сверки состояния','Retrying the game action once after state refresh'),'info');
      try{
        return await request();
      }catch(secondError){
        if(secondError?.name==='AbortError')throw secondError;
        recordDiagnostic('business-canon-second-error',{action,buildingId:row.buildingId,slot:row.slot,businessId:expected,name,error:secondError?.message||String(secondError),status:Number(secondError?.httpStatus||0),apiData:secondError?.apiData||null});
        hkRunner.note(either('Повторная ошибка — останавливаю текущую пару','Second game error — stopping the current pair'),'bad');
        throw secondError;
      }
    }
  }

  async function businessCanonicalCurrent(row,reason='business-canon-current') {
  ...
  async function insertAndActivateBusinessCanonical(row,businessId,label='') {
    const expected=String(businessId||'');
    const name=label||businessDisplayName(expected);
    await businessCanonicalMutation('insert',row,expected,name);
    let current=await businessCanonicalCurrent(row,'business-canon-after-insert');
    if(!current||String(current.businessId||'')!==expected){
      playerDocument=await hkAuthoritativePlayerRead('business-canon-insert-verify');
      current=findSlot(playerDocument,row.buildingId,row.slot);
    }
    if(!current||String(current.businessId||'')!==expected)throw new Error(either('После вставки бизнес не найден в слоте','Inserted business is missing from the slot'));

    let timer=Math.max(0,Number(current.timer||0));
    const free=Math.max(0,Number(current.freeSpeedUpTime||0));
    if(timer>0){
      if(free<timer)throw new Error(either('Бесплатное ускорение ещё недоступно для «'+name+'»','Free speed-up is not yet available for '+name));
      hkRunner.setStep(either('Ускоряю: ','Speeding up: ')+name,hkRunner.state.done,hkRunner.state.total);
      await businessCanonicalMutation('speedUp',current,expected,name,timer);
      current=await businessCanonicalCurrent(row,'business-canon-after-speedup');
      timer=Math.max(0,Number(current?.timer||0));
      if(timer>0){
        playerDocument=await hkAuthoritativePlayerRead('business-canon-speedup-verify');
        current=findSlot(playerDocument,row.buildingId,row.slot);
      }
    }

    if(String(current?.status||'').toUpperCase()!=='ACTIVE'){
      hkRunner.setStep(either('Активирую: ','Activating: ')+name,hkRunner.state.done,hkRunner.state.total);
      await businessCanonicalMutation('activate',current||row,expected,name);
      current=await businessCanonicalCurrent(row,'business-canon-after-activate');
    }
    if(!businessSlotIsActive(current,expected)){
      playerDocument=await hkAuthoritativePlayerRead('business-canon-final-pair-verify');
      current=findSlot(playerDocument,row.buildingId,row.slot);
    }
    if(!businessSlotIsActive(current,expected))throw new Error(either('Не подтверждена активация «'+name+'»','Activation was not confirmed for '+name));
    return current;
  }

  async function restoreBusinessPairCanonical(row,originalBusinessId,incomingBusinessId='') {
    try{
      playerDocument=await hkAuthoritativePlayerRead('business-canon-pair-rollback-start');
      let current=findSlot(playerDocument,row.buildingId,row.slot);
      if(current?.businessId){
        await businessCanonicalMutation('remove',current,current.businessId,businessDisplayName(current.businessId));
        playerDocument=await hkAuthoritativePlayerRead('business-canon-pair-rollback-removed');
        current=findSlot(playerDocument,row.buildingId,row.slot);
      }
      if(originalBusinessId){
        await insertAndActivateBusinessCanonical(row,originalBusinessId,businessDisplayName(originalBusinessId));
      }
      hkRunner.note(either('Текущий слот возвращён к исходному состоянию','Current slot restored to its original state'),'ok');
      return true;
    }catch(rollbackError){
      recordDiagnostic('business-canon-pair-rollback-failed',{buildingId:row.buildingId,slot:row.slot,originalBusinessId,incomingBusinessId,error:rollbackError?.message||String(rollbackError)});
      hkRunner.note(either('Не удалось восстановить текущий слот: ','Could not restore current slot: ')+String(rollbackError?.message||rollbackError),'bad');
      return false;
    }
  }

  function businessCanonicalPreflight(plan) {
    const workers=businessWorkerState(playerDocument);
    if(workers.max>0&&workers.busy>=workers.max)throw new Error(either('Нет свободного бизнес-менеджера','No free business manager'));

    const needed=new Map();
    for(const [,id] of plan)if(id)needed.set(String(id),(needed.get(String(id))||0)+1);
    const stock=new Map(inventory.map(row=>[String(row.businessId),Math.max(0,Number(row.quantity||0))]));
    for(const [id,count] of needed)if((stock.get(id)||0)<count)throw new Error(either('Состав склада изменился — перечитайте бизнесы','Business stock changed — refresh Businesses'));

    for(const [row] of plan){
      const live=findSlot(playerDocument,row.buildingId,row.slot);
      const expected=String(row.businessId||'');
      const current=String(live?.businessId||'');
      if(!live||current!==expected||(current&&(!businessSlotIsActive(live,current))))throw new Error(either('Состояние слота изменилось — перечитайте бизнесы','Slot state changed — refresh Businesses'));
    }
    return true;
  }
  ...
  async function executeBusinessPlan() {
    ...
    if (!confirm(language === 'en'
      ? `Rearrange ${plan.length} businesses?\n\nKokkaras canon: each slot is completed before the next one. Only free 0-cost speed-up is allowed. Crystals are prohibited.`
      : `Переставить ${plan.length} бизнесов?\n\nКанон Kokkaras: каждый слот завершается полностью до перехода к следующему. Разрешено только бесплатное ускорение за 0. Кристаллы запрещены.`)) return;
    if (hkRunner.running) { alert(either('Сначала завершите текущую задачу','Finish the current task first')); return; }

    businessBusy=true;
    let completed=0,failed=0,currentPair=null;
    try{
      log(either('Проверяю игровую сессию и план…','Checking game session and plan…'));
      playerDocument=await apiJson('/player/me','POST');
      await ensureRecipeMetadata();
      refreshBusinessData();
      plan=makePlan();
      businessCanonicalPreflight(plan);
      ...
      for(let index=0;index<plan.length;index++){
        if(hkRunner.signal?.aborted)throw new DOMException('Aborted','AbortError');
        await hkRunner.waitIfPaused();
        const [row,id]=plan[index];
        currentPair={row,id,index};
        const oldName=row.businessId?businessDisplayName(row.businessId):either('Пустой слот','Empty slot');
        const newName=id?businessDisplayName(id):either('Пустой слот','Empty slot');
        hkRunner.setStep(either('Снимаю: ','Removing: ')+oldName,index,plan.length);

        try{
          if(row.businessId){
            await businessCanonicalMutation('remove',row,row.businessId,oldName);
            log('− '+oldName+' — '+buildingSlotLabel(row),'info');
          }

          if(id){
            hkRunner.setStep(either('Вставляю: ','Inserting: ')+newName,index,plan.length);
            await insertAndActivateBusinessCanonical(row,id,newName);
            log('✓ '+newName+' — '+buildingSlotLabel(row),'ok');
          }else{
            log(either('✓ Слот оставлен пустым — ','✓ Slot left empty — ')+buildingSlotLabel(row),'ok');
          }
          completed++;
        }catch(pairError){
          failed++;
        ...
        hkRunner.setStep(either('Готово: ','Done: ')+newName,index+1,plan.length);
        currentPair=null;
        if(index<plan.length-1)await gameRetryDelay(500);
      }

      playerDocument=await hkAuthoritativePlayerRead('business-canon-final-run-verify');
      const mismatches=[];
      for(const [row,id] of plan){
        const state=findSlot(playerDocument,row.buildingId,row.slot);
        if(id){
          if(!businessSlotIsActive(state,id))mismatches.push({row,id,state});
        }else if(state?.businessId){
          mismatches.push({row,id,state});
        }
      }
      if(mismatches.length)throw new Error(either('Финальная проверка не подтверждена для '+mismatches.length+' слотов','Final verification failed for '+mismatches.length+' slots'));

      refreshBusinessData();
      hkRunner.finish(either('Перестановка завершена','Business rearrangement completed'));
      selectedSlots.clear(); selectedStock.clear(); preparedBusinessPlan=null; preparedBusinessPlanSource='';
      log(either('Перестановка завершена. Успешно: ','Rearrangement completed. Success: ')+completed+', '+either('ошибок: ','errors: ')+failed,'ok');
      showVisualNotice(either('Перестановка бизнесов завершена','Business rearrangement completed'),either('Успешно переставлено: ','Successfully rearranged: ')+completed);
    }catch(error){
  ...
  function renderRunnerState() {
    ...
    const runnerFill=root.querySelector('#hk-runner-fill');
    if(businessesRun){runnerFill.style.width='100%';runnerFill.style.height=`${percent}%`;}else{runnerFill.style.width=`${percent}%`;runnerFill.style.height='100%';}
...
.hk-growth-stats{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin:10px 0}.hk-growth-stats>div{padding:11px;border:1px solid #304057;border-radius:12px;background:#0e1723}.hk-growth-stats small{display:block;color:#8797ad;font-size:10px}.hk-growth-stats b{display:block;margin-top:4px;color:#ffe083;font-size:15px}.hk-growth-actions{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.hk-growth-actions button{margin-top:0}.hk-growth-options{display:grid;gap:9px;margin:10px 0}.hk-growth-option{display:grid;grid-template-columns:minmax(0,1fr) 110px;gap:10px;align-items:center;padding:10px;border:1px solid #2d3a4e;border-radius:12px;background:#101927}.hk-growth-option span{display:grid;gap:3px}.hk-growth-option small{color:#8998ad;font-size:10px}.hk-growth-option select{background:#0b111b;color:white;border:1px solid #3b4a61;border-radius:10px;padding:9px}.hk-growth-check{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:10px;border:1px solid #2d3a4e;border-radius:12px;background:#101927}.hk-growth-check input{width:20px;height:20px;accent-color:#ffad1f}.hk-growth-live{display:grid;gap:3px;padding:9px 10px;border:1px solid #245d48;border-radius:11px;background:#0d211b}.hk-growth-live span{font-size:11px;font-weight:900;color:#6ee7a8}.hk-growth-live small{font-size:9px;color:#8fa89f}.hk-growth-plan{display:grid;gap:6px;margin:4px 0 10px;padding:11px;border:1px solid #3a4659;border-radius:12px;background:#0e1723}.hk-growth-plan>b{color:#ffe083}.hk-growth-plan>span{font-size:11px;color:#d4deea}.hk-growth-plan>small{font-size:9px;color:#8290a4;line-height:1.4}.hk-growth-priority-head{display:flex;align-items:center;justify-content:space-between;gap:8px}.hk-growth-priority-head h3{margin:0}.hk-growth-priority-list{display:grid;gap:7px;margin-top:8px}.hk-growth-priority{display:grid;grid-template-columns:64px minmax(0,1fr) auto;align-items:center;gap:8px;padding:8px;border:1px solid #2d3a4e;border-radius:10px;background:#101927}.hk-growth-priority>b{color:#ffe083}.hk-growth-priority>span{font-size:9px;color:#8797ad}.hk-growth-priority>div{display:flex;gap:4px}.hk-growth-priority button{min-width:30px;margin:0;padding:6px 8px}.hk-growth-stats>div:nth-child(n+5){background:#121a28}@media(max-width:430px){.hk-tab{min-height:58px;padding:8px}.hk-tab-icon{width:32px;height:32px;font-size:19px}}
      .hk-business-tabs{display:grid;grid-template-columns:repeat(3,1fr);gap:7px;margin:10px 0}.hk-business-tab{border:1px solid #34445b;border-radius:11px;padding:11px;background:#172234;color:#b8c4d6;font-weight:700}.hk-business-tab.active{border-color:#ffad1f;background:#3a2a0f;color:#fff}.hk-business-pane{display:none}.hk-business-pane.active{display:block}
    ...
    // Canonical Pits controls are rendered and bound dynamically by pitCanonRender().
  ...
  window.addEventListener('pagehide',()=>{if(publicCollectorActivityLastActive===true)void syncPublicCollectorActivity(false,'pagehide',true);},{capture:true});
